...
with col_dir:
    ...
    with st.expander("🔮 Simulador de Meses Futuros"):
        st.markdown("**Projeção de Meses Futuros (Com Caixinhas CDI)**")
        
        historico = dados.get("historico", {})
        somas_hist = []
        for mes_ant, dm in historico.items():
            tot_m = sum(sum(i["valor"] for i in d.get("itens", [])) for d in dm.get("despesas", []))
            if tot_m > 0:
                somas_hist.append(tot_m)
        
        tot_atual = sum(sum(item["valor"] for item in d.get("itens", [])) for d in dados.get("despesas", []))
        if tot_atual > 0:
            somas_hist.append(tot_atual)
            
        media_despesas = sum(somas_hist) / len(somas_hist) if somas_hist else tot_atual
        salario_atual = sum(i["valor"] for i in dados.get("receitas", []))
        
        despesas_sim = copy.deepcopy(dados.get("despesas", []))
        caixinhas_sim = copy.deepcopy(dados.get("caixinhas", []))
        
        meses_lista_pt = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"]
        
        try:
            mes_atual_str = dados.get("mes_atual", "Janeiro / 2026")
            partes = mes_atual_str.split("/")
            nome_mes_atual = partes[0].strip()
            ano_atual = int(partes[1].strip())
            idx_base = meses_lista_pt.index(nome_mes_atual)
        except Exception:
            idx_base = datetime.now().month - 1
            ano_atual = datetime.now().year

        for m_offset in range(1, 7):
            idx_fut = (idx_base + m_offset) % 12
            ano_fut = ano_atual + ((idx_base + m_offset) // 12)
            nome_mes_fut = f"{meses_lista_pt[idx_fut]} / {ano_fut}"
            ...
            for cx in caixinhas_sim:
                ...
                novo_tot_cx, _ = calcular_rendimento_caixinha(din_ini, ap_men, cdi_anual, dias_investido=30 * m_offset)
                cx["dinheiro_inicial"] = novo_tot_cx
                tot_cx_sim += novo_tot_cx
                tot_ap_sim += ap_men

            tot_parc = 0.0
            novas_desp_sim = []
            for d in despesas_sim:
                novos_itens_s = []
                for item in d.get("itens", []):
                    parc = item.get("parcelas", 1)
                    if parc > 0:
                        tot_parc += item["valor"]
                        if parc > 1:
                            ni = copy.deepcopy(item)
                            ni["parcelas"] = parc - 1
                            novos_itens_s.append(ni)
                novas_desp_sim.append({"categoria": d["categoria"], "meta": d.get("meta", 0.0), "itens": novos_itens_s})
            
            despesas_sim = novas_desp_sim
            tot_gastos_proj = media_despesas + tot_parc
            saldo_proj = salario_atual - tot_gastos_proj - tot_ap_sim

            st.markdown(f"📅 **{nome_mes_fut}**")
            st.text(f"  • Gastos Proj.: R$ {tot_gastos_proj:.2f} | Aportes: R$ {tot_ap_sim:.2f}")
            st.text(f"  • Caixinhas Acumuladas (CDI): R$ {tot_cx_sim:.2f}")
            st.markdown(f"  • **Saldo Livre Estimado: R$ {saldo_proj:.2f}**")
            st.divider()
